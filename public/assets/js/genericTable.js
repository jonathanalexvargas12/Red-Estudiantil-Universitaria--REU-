class GenericTable {
    constructor(tableName, tableId, columns, displayColumns, options = {}) {
        ...
        this.tableType = tableElement?.getAttribute('data-table-type') || tableName;
    ...
    asignarEventosEditarEliminar() {
        document.querySelectorAll(`#${this.tableId} .editar-icono`).forEach(btn => {
            btn.onclick = (e) => {
                ...
                const item = this._storedData[id];
                if (this.onEditar) this.onEditar(id, celdas, item);
            };
        });
    ...
    setupEventDelegation() {
        ...
        tablaElement.addEventListener('click', (event) => {
            ...
            if (target.classList.contains('editar-icono') || target.closest('.editar-icono')) {
                ...
                const item = this._storedData[id];
                if (this.onEditar) this.onEditar(id, datos, item);
                return;
            }
            ...
            if (this.tableName === 'bitacora' && (target.classList.contains('detalle-bitacora-icono') || target.closest('.detalle-bitacora-icono'))) {
                const icono = target.classList.contains('detalle-bitacora-icono') ? target : target.closest('.detalle-bitacora-icono');
                const id = icono.getAttribute('data-id');
                if (this.onDetalleBitacora) this.onDetalleBitacora(id);
                return;
            }
        });
    }
    ...
    actualizarTabla(datos) {
        ...
        datos.forEach(item => {
            ...
            this._storedData[item[pk]] = item;
            const nuevaFila = document.createElement('tr');

            this.displayColumns.forEach(col => {
                let contenido = item[col];
                if (contenido === null || contenido === undefined || contenido === '') contenido = 'Sin datos';
            ...
            if (this.tableName === 'bitacora') {
                nuevaFila.innerHTML += `<td class="acciones-cell">
                    <i class="fa-solid fa-eye accion-icono detalle-bitacora-icono" 
                       title="Ver Detalles" 
