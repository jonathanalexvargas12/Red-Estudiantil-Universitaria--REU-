...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Métodos para editar
    tablaCalificaciones.onEditar = async (id, datos, item) => {
        ...
        periodoSelect.value = item.Periodo_Academico || datos[1] || '';
        asignaturaSelect.value = item.Unidad_Curricular || datos[2] || '';
        seccionSelect.value = item.Seccion || datos[3] || '';
        profesorInput.value = item.Nombre_Docente || datos[4] || '';
        estudianteInput.value = item.Nombre_Estudiante || datos[5] || '';
        notaInput.value = item.Calificacion_Numerica || datos[6] || '';
