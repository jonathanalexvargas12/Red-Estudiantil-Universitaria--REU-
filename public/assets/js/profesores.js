...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Métodos para editar
    tablaProfesores.onEditar = (id, datos, item) => {
        ...
        inputs[3].value = (item.Tipo || datos[3] || '').toLowerCase();
        inputs[4].value = (item.Estado_Docente || datos[4] || '').toLowerCase();
        
        // Precargar campos adicionales si existen en el formulario
        const telefono1 = document.getElementById('editar-telefono-1');
        const telefono2 = document.getElementById('editar-telefono-2');
        const correo = document.getElementById('editar-correo');
        const codigoCarnet = document.getElementById('editar-codigo-carnet');
        if (telefono1) telefono1.value = item.Telefono_1 || '';
        if (telefono2) telefono2.value = item.Telefono_2 || '';
        if (correo) correo.value = item.Correo || '';
        if (codigoCarnet) codigoCarnet.value = item.Codigo_Carnet || '';
