...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const generarPDF = async (datos) => {
        ...
        const columnConfig = [
            ...
            { header: 'Estado Pensum', dataKey: 'Estado_Pensum', width: 30 },
            { header: 'Estado', dataKey: 'Estado', width: 20 },
            { header: 'Código Carnet', dataKey: 'Codigo_Carnet', width: 30 }
        ];
    ...
    // Función para obtener datos de usuario por cédula
    ...
    // Función para generar la ficha del estudiante en PDF (combinando datos de estudiantes y usuarios)
    const generarFichaEstudiantePDF = async (estudiante, datosUsuario) => {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF({
        ...
        yPos += 5;
        doc.text(`• Estado: ${estudiante.Estado === 'regular' ? 'Regular' : 
                 estudiante.Estado === 'nuevo-ingreso' ? 'Nuevo Ingreso' : 'N/A'}`, margin, yPos);
        yPos += 5;
        doc.text(`• Código de Carnet: ${estudiante.Codigo_Carnet || 'N/A'}`, margin, yPos);
        yPos += 10;

        // Datos de contacto (obtenidos de la tabla usuarios)
        doc.setFont('helvetica', 'bold');
        doc.text('Datos de Contacto:', margin, yPos);
        yPos += 7;

        doc.setFont('helvetica', 'normal');
        doc.text(`• Teléfono 1: ${datosUsuario.Telefono_1 || 'N/A'}`, margin, yPos);
        yPos += 5;
        doc.text(`• Teléfono 2: ${datosUsuario.Telefono_2 || 'N/A'}`, margin, yPos);
        yPos += 5;
        doc.text(`• Correo: ${datosUsuario.Correo || 'N/A'}`, margin, yPos);
        yPos += 10;
    ...
    tablaEstudiantes.onFichaEstudiante = async (cedula) => {
        try {
            ...
            for (const fila of filas) {
                if (fila.cells[0].textContent.trim() === cedula) {
                    estudiante = {
                        ...
                        // Para obtener la carrera necesitamos acceder a los datos completos
                        Carrera: fila.dataset.carrera || 'N/A' // Asumiendo que agregamos data-carrera a las filas
                    };
                    break;
                }
            }
            ...
            const datosUsuario = await obtenerDatosUsuario(cedula);

            // Generar la ficha combinando ambos conjuntos de datos
            await generarFichaEstudiantePDF(estudiante, datosUsuario);
    ...
    // Inicialización
    tablaEstudiantes.cargarDatos().then(() => {
        // Agregar data-carrera a cada fila para facilitar el acceso
        const filas = document.querySelectorAll('#tabla-estudiantes tbody tr');
        filas.forEach(fila => {
            const cedula = fila.cells[0].textContent.trim();
            // Asumimos que la carrera está disponible en los datos
            // Si no, necesitaríamos modificar el GenericTable para incluirla
            fila.dataset.carrera = fila.dataset.carrera || 'N/A';
        });
