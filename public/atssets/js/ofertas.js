...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tablaDocenteAsignatura = new GenericTable(
        ...
        ['Docente_Cedula', 'Docente_Nombre', 'Carrera', 'Pensum', 'Trayecto', 'Sem/Trim', 'Asignatura', 'Seccion', 'Num_Alumnos', 'Clases_Semana', 'Estado', 'Periodo_Academico']
    );
    ...
    const filtrarTabla = () => {
        ...
        filasOriginales.forEach(filaOriginal => {
            ...
            // Cambio aquí: usar comparación estricta en lugar de includes
    ...
    tablaDocenteAsignatura.cargarDatos = async function() {
        ...
        // Llenar selectores de filtro y formularios con datos de otras tablas
        await cargarSelectoresExternos();
    };

    // 11. Función para cargar todos los selectores desde APIs externas
    const cargarSelectoresExternos = async () => {
        try {
            const token = getToken();
            if (!token) throw new Error('No se encontró el token JWT');

            // Obtener datos de todas las tablas necesarias
            const [periodos, carreras, asignaturas, pensums, niveles, secciones] = await Promise.all([
                fetch('/api/periodo_academico', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json()),
                fetch('/api/carreras', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json()),
                fetch('/api/asignaturas', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json()),
                fetch('/api/pensum', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json()),
                fetch('/api/nivel_pensum', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json()),
                fetch('/api/secciones', { headers: { 'Authorization': `Bearer ${token}` } }).then(res => res.json())
            ]);

            // Función para llenar un selector con datos
            const llenarSelector = (selectorId, datos, campoValor, campoTexto = null, placeholder = 'Seleccione una opción') => {
                const selector = document.getElementById(selectorId) || document.querySelector(selectorId);
                if (!selector) return;
                ...
                // Opción "Todos" para los filtros
                if (selectorId.includes('filtro') || selectorId.includes('select-')) {
                ...
                datos.forEach(item => {
                    ...
                    option.textContent = campoTexto ? `${item[campoTexto]} (${item[campoValor]})` : item[campoValor];
                    selector.appendChild(option);
                });
            };

            // Llenar selectores de FILTROS
            llenarSelector('.select-periodo-ofertas', periodos, 'Periodo_Academico', null, 'Periodo');
            llenarSelector('.select-carrera-ofertas', carreras, 'Codigo_Carrera', 'Nombre_Carrera', 'Carrera');

            // Llenar selectores de FORMULARIOS (agregar/editar)
            llenarSelector('agregar-periodo', periodos, 'Periodo_Academico');
            llenarSelector('editar-periodo', periodos, 'Periodo_Academico');
            llenarSelector('agregar-asignatura', asignaturas, 'Codigo_Asignatura', 'Nombre_Asignatura');
            llenarSelector('editar-asignatura', asignaturas, 'Codigo_Asignatura', 'Nombre_Asignatura');
            ...
            llenarSelector('agregar-carrera', carreras, 'Codigo_Carrera', 'Nombre_Carrera');
            llenarSelector('editar-carrera', carreras, 'Codigo_Carrera', 'Nombre_Carrera');
    ...
    // 14. Agregar nueva asignación docente-asignatura
