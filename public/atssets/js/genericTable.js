class GenericTable {
    constructor(tableName, tableId, columns, displayColumns, options = {}) {
        ...
        // Opciones por defecto (compatibilidad con version anterior)
        this.disableEdit = options.disableEdit || false;
        this.disableDelete = options.disableDelete || false;
        
        // Si options es un booleano (para mantener compatibilidad)
    ...
    // Método para asignar eventos de editar/eliminar (compatibilidad)
    ...
    setupEventDelegation() {
        ...
        tablaElement.addEventListener('click', (event) => {
            ...
            // Manejar evento de PDF para pensum
            if (this.tableName === 'pensum' && (target.classList.contains('pdf-icono-pensum') || target.closest('.pdf-icono-pensum'))) {
    ...
    actualizarTabla(datos) {
        ...
        datos.forEach(item => {
            ...
            this.displayColumns.forEach(col => {
                ...
                if (col === 'Inicio' || col === 'Final' || col === 'Fecha_Registro') {
            ...
            // Para la bitácora, no generamos acciones
            if (this.tableName !== 'bitacora') {
                nuevaFila.innerHTML += this.generarAccionesHTML(item);
            } else {
                // Para bitácora, solo añadimos una celda vacía para mantener la estructura
    ...
    generarAccionesHTML(item) {
        ...
        if (this.tableName === 'estudiantes') {
            ...
            // Mostrar icono de editar solo si no está deshabilitado
            if (!this.disableEdit) {
                accionesHTML += `<i class="fas fa-edit accion-icono editar-icono" title="Editar" data-id="${item[this.columns[0]]}"></i>`;
            }
            
            // Mostrar icono de eliminar solo si no está deshabilitado
